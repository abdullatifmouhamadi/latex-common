...
\fancyfoot[L]{\arb{.Al-llu.gaT .Al`arabyyaT}}
...
%\newfontfamily\arabicfont[Script=Arabic, Scale=1.5]{Noto Naskh Arabic}
%\newfontfamily\arabicfont[Script=Arabic, Scale=1.5]{Amiri}
\newfontfamily\arabicfont[Script=Arabic, Scale=1.5]{ScheherazadeNew}
%\newfontfamily\arabicfont[Script=Arabic, Scale=1.5]{Harmattan}
...
%%%%%%%%%%%%%% ARRAY DIALOGUES
\def\dialogacolor{general@best@olive}
\newcommand{\dialoga}[1]{%
...
\begin{tikzpicture}
  ...
  \node[white, fill=general@best@orange!100!50, signal to=west, signal from=east] at (0,1) {\bfseries\normalsize#1};
\end{tikzpicture}
...
\newcommand{\arem}[1]{\arbcolor[\arbremacolor]{\arb[fullvoc]{#1}}}
\newcommand{\aans}[1]{\arbcolor[general@best@purple]{\arb{#1}}}
\newcommand{\arbrem}[1]{\arbcolor[\arbremacolor]{\arb{#1}}}
...
% https://www.latex4technics.com/?note=nnf
% https://www.latex4technics.com/?note=JR7

\newcommand{\arblex}[2]{
    \begin{longtable}{C{.04\linewidth}R{.195\linewidth}L{.195\linewidth}}
      \arrayrulecolor{gray}
        %\hline
        \rowcolor{general@best@maroon}\multicolumn{3}{c}{\textcolor{white}{\bfseries #1}}\\
        %\textbf{~} & \textbf{arb} & \textbf{fr}\\\hline
        %\DTLforeach[\DTLiseq{\key}{#1}]{datas}{% 
        %        \key=key,%
        %        \unit=unit,%
        %        \arabic=arabic,% 
        %        \french=french%
...
\newcommand{\alr}[4]{
    \cellcolor{general@best@orange!16}{#1}&\cellcolor{general@best@yellow!16}{#2}&#3\\\hline
}
