...
\fancypagestyle{main}{% 
	...
	\fancyfoot[R,L]{\basPageTitre}
  	...
  	\fancyfoot[LE]{% pages paires	
  		...
  		%\vskip0.1pt
  		\begin{tikzpicture}
  		\node[rounded corners=3pt,fill=shadow@color,inner ysep=5pt, inner xsep=2ex,text=shadow@color] (shadow) {\pagenumber@font\thepage~/~\pageref*{LastPage}};
  		\node[rounded corners=3pt,fill=pagenumber@bg@color,inner ysep=5pt, inner xsep=2ex,text=white] at ($(shadow)+(1pt,1pt)$) {\pagenumber@font\thepage~/~\pageref*{LastPage}};
  		\end{tikzpicture}
  	}
}
