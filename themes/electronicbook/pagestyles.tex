...
\def\basPageTitre{Dossier ressource v1.0}

\fancypagestyle{chapterstart}{% 1ère page des chapitres
	\renewcommand{\headrulewidth}{0pt}
	\fancyhf{}%
	\fancyfoot[R,L]{\basPageTitre}
  	\fancyfoot[RO,RE]{%
  	%\vskip0.1pt
  	\begin{tikzpicture}
  	  %\node[rounded corners=4pt,fill=shadow@color,inner ysep=5pt, inner xsep=2ex,text=shadow@color] (shadow) {\pagenumber@font\thepage~/~\pageref*{LastPage}};
	  %\node[rounded corners=4pt,fill=pagenumber@bg@color,inner ysep=5pt, inner xsep=2ex,text=white] at ($(shadow)+(-1pt,1pt)$) {\pagenumber@font\thepage~/~\pageref*{LastPage}};
	  \node[rounded corners=4pt,fill=pagenumber@bg@color,inner ysep=0pt, inner xsep=2ex,text=black] {\pagenumber@font\thepage~/~\pageref*{LastPage}};
  	\end{tikzpicture}
  	}
}
...
\fancypagestyle{main}{% 
	\fancyhf{}%
	\fancyhead[LO,RE]{\includegraphics[scale=0.02]{common/themes/electronicbook/uimm.png}}
	%\fancyhead[R]{UIMM}
	%\fancyhead[LE,RO]{\rightmark} --old
	\fancyhead[LE,RO]{\slshape\nouppercase{\rightmark}} % minuscule
	\fancyfoot[R,L]{\basPageTitre}
  	...
  	%\node[rounded corners=4pt,fill=shadow@color,inner ysep=5pt, inner xsep=2ex,text=shadow@color] (shadow) {\pagenumber@font\thepage~/~\pageref*{LastPage}};
  	%\node[rounded corners=4pt,fill=pagenumber@bg@color,inner ysep=5pt, inner xsep=2ex,text=white] at ($(shadow)+(-1pt,1pt)$) {\pagenumber@font\thepage~/~\pageref*{LastPage}};
	\node[rounded corners=4pt,inner ysep=0pt, inner xsep=2ex,text=black] {\pagenumber@font\thepage~/~\pageref*{LastPage}};  
	\end{tikzpicture}
  	}
...
%\renewcommand{\footrule}{\hbox to \headwidth{\color{easi@default@blue}\leaders\hrule height \footrulewidth\hfill}}
%\renewcommand{\headrule}{\hbox to \headwidth{\color{easi@default@blue}\leaders\hrule height \headrulewidth\hfill}}
...
\newcommand{\formatchapter}[1]{%
	\begin{tikzpicture}[remember picture,overlay]
		...
		\node[anchor=west,xshift=4.3cm,yshift=1.25cm, rectangle, rounded corners=12pt,inner sep=30pt, fill=easi@default@blue, minimum width=.76\paperwidth] {\color{white} #1};
		\end{tikzpicture}
		};
	\end{tikzpicture}
}
